...
class BusinessRegistrationService {
  ...
  Future<void> registerNowOrQueue(Map<String, dynamic> payload) async {
    try {
      await api.register(baseUrl: kLicenseBackendBaseUrl, payload: payload);
      return;
    } catch (e) {
  ...
  Future<void> retryPendingOnce() async {
    ...
    for (final item in items) {
      ...
      try {
        await api.register(baseUrl: kLicenseBackendBaseUrl, payload: item.payload);
        await queue.removeById(item.id);
      } catch (e) {
        await queue.replaceItem(next.copyWith(lastError: e.toString()));
      }
    }
  }
}
